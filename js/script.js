...
$(document).on('click', '#bsn-elimina-profilo-btn', function() {
    ...
    $(document).on('click', '.bsn-risultato-articolo', function() {
        ...
        var kit = bsnGetKitByArticoloId(id);
        if (kit) {
            bsnAttachKitToRow($row, kit);
        }
            ...
            success: function(response) {
                ...
                lista.forEach(function(n) {
                    ...
                    if (stato === 'bozza') {
                        ...
                        classeStato = 'bsn-stato-preventivo';
                    ...
                    var btnIspeziona = '<a href="' + window.location.origin +
                        '/ispeziona/?id=' +
                        encodeURIComponent(n.id) +
                        '" class="btn bsn-noleggio-ispeziona" ' +
                        'title="Ispeziona noleggio">👁️ Ispeziona</a>';
                    var btnElimina = '<button type="button" class="btn bsn-noleggio-delete" title="Elimina noleggio" ' +
                    ...
                    html += '' +
                        '<tr class="bsn-noleggio-row ' + classeStato + '" data-id="' + n.id + '" data-stato="' + stato + '">' +
                            '<td>' + n.id + '</td>' +
                                ...
                                '<div class="bsn-azioni-grid">' +
                                    (stato === 'preventivo'
                                        ...
                                        : (
                                            btnDettagli +
                                            btnDuplica +
                                            // Rientro (Chiudi) per attivo/ritardo o scorciatoia Ticket per chiuso
                                            ((stato === 'attivo' || stato === 'ritardo')
                                                ? '<button type="button" class="button button-secondary bsn-noleggio-rientro" ' +
                                                    'title="Chiudi / Rientro noleggio">🚚 Rientro</button>'
                                                : (stato === 'chiuso'
                                                    ? '<button type="button" class="btn bsn-noleggio-ticket-shortcut" ' +
                                                        'data-noleggio-id="' + n.id + '" title="Apri ticket">🎫 Ticket</button>'
                                                    : ''
                                                )
                                            ) +
