...
$sql_articoli = "CREATE TABLE $table_articoli (
    ...
    // Ticket (anomalie rientro o segnalazioni manuali)
        ...
        btn.addEventListener('click', function(e){
            ...
            .then(function(data){
                if (data && data.ticket_count && parseInt(data.ticket_count, 10) > 0) {
                    alert('Rientro salvato. Ci sono articoli con Ticket da compilare.');
                    window.location.href = '<?php echo esc_js( home_url( '/tiket-rientri/' ) ); ?>?id=' + encodeURIComponent(idNoleggio);
                } else {
...
add_shortcode( 'blackstar_tiket', 'bsn_shortcode_tiket_rientri' );
function bsn_shortcode_tiket_rientri() {
    if ( ! bsn_check_admin() ) {
        return '<p>Accesso negato. Permesso riservato agli operatori autorizzati.</p>';
    }
    ...
    $transient_key = $id_noleggio ? 'bsn_ticket_rientro_' . $id_noleggio . '_' . $user_id : '';

    $ticket_items = [];
    if ( $transient_key ) {
        $ticket_items = get_transient( $transient_key );
    }

    if ( empty( $ticket_items ) || ! is_array( $ticket_items ) ) {
        return '<p>Nessun articolo da ticket per questo rientro.</p>';
    }

    global $wpdb;
    $table_articoli = $wpdb->prefix . 'bs_articoli';
    $ids_articoli = array_values( array_unique( array_map( function( $item ) {
        return intval( $item['articolo_id'] );
    }, $ticket_items ) ) );
    ...
    ?>
    <div class="wrap">
        <h1>Ticket da rientro <?php echo esc_html( $id_noleggio ); ?></h1>
        <p>Compila i dettagli per ogni articolo con anomalie.</p>

        <form id="bsn-form-ticket-rientri">
            <div id="bsn-ticket-rientri-list">
                <?php foreach ( $ticket_items as $index => $item ) :
                    $articolo_id = intval( $item['articolo_id'] );
                    $qty_noleggiata = intval( $item['qty'] );
                    $qty_rientrata = intval( $item['qty_rientrata'] );
                    $qty_default = max( 1, $qty_noleggiata - $qty_rientrata );
                    $label = 'Articolo #' . $articolo_id;
                    if ( isset( $mappa_articoli[ $articolo_id ] ) ) {
                        $info = $mappa_articoli[ $articolo_id ];
                        $label = trim( ( $info['codice'] ? '[' . $info['codice'] . '] ' : '' ) . $info['nome'] );
                    }
                    ?>
                    <div class="bsn-ticket-rientro-row" style="border:1px solid #ddd; padding:12px; margin-bottom:12px; background:#fff;">
                        <div><strong><?php echo esc_html( $label ); ?></strong></div>
                        <input type="hidden" name="ticket_items[<?php echo esc_attr( $index ); ?>][articolo_id]" value="<?php echo esc_attr( $articolo_id ); ?>">
                        <input type="hidden" name="ticket_items[<?php echo esc_attr( $index ); ?>][noleggio_id]" value="<?php echo esc_attr( $id_noleggio ); ?>">

                        <div style="display:flex; gap:12px; flex-wrap:wrap; margin-top:8px;">
                            <div style="flex:0 0 140px;">
                                <label>Quantità *</label>
                                <input type="number" min="1" name="ticket_items[<?php echo esc_attr( $index ); ?>][qty]" value="<?php echo esc_attr( $qty_default ); ?>" style="width:100%;">
                            </div>
                            <div style="flex:1 1 220px;">
                                <label>Tipo problema *</label>
                                <select name="ticket_items[<?php echo esc_attr( $index ); ?>][tipo]" style="width:100%;">
                                    <option value="mancante">Mancante</option>
                                    <option value="non_rientrato">Non rientrato</option>
                                    <option value="quantita_inferiore">Quantità inferiore</option>
                                    <option value="danneggiato">Danneggiato</option>
                        ...
                        <div style="margin-top:8px;">
                            <label>Note operative *</label>
                            <textarea name="ticket_items[<?php echo esc_attr( $index ); ?>][note]" rows="2" style="width:100%;"><?php echo esc_textarea( $item['note'] ); ?></textarea>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
            <button type="submit" class="button button-primary">Salva Ticket</button>
        </form>
    </div>

    <script>
    (function() {
        const form = document.getElementById('bsn-form-ticket-rientri');
        if (!form) return;

        form.addEventListener('submit', function(e) {
            e.preventDefault();
            const rows = form.querySelectorAll('.bsn-ticket-rientro-row');
            const items = [];
            let hasError = false;

            rows.forEach(function(row) {
                const articoloId = row.querySelector('input[name$="[articolo_id]"]').value;
                const noleggioId = row.querySelector('input[name$="[noleggio_id]"]').value;
                const qtyInput = row.querySelector('input[name$="[qty]"]');
                const tipoSelect = row.querySelector('select[name$="[tipo]"]');
                const noteInput = row.querySelector('textarea[name$="[note]"]');

                const qty = qtyInput ? parseInt(qtyInput.value, 10) || 0 : 0;
                const tipo = tipoSelect ? tipoSelect.value : '';
                const note = noteInput ? noteInput.value.trim() : '';

                if (!articoloId || qty <= 0 || !tipo || !note) {
                    hasError = true;
                    return;
                }
            ...
            if (hasError) {
                alert('Compila tutti i campi obbligatori per ogni ticket.');
                return;
            }
            ...
            Promise.all(items.map(function(item) {
                return fetch(BSN_API.root + 'tickets', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-WP-Nonce': BSN_API.nonce
                ...
                alert('Ticket salvati correttamente.');
                window.location.href = '<?php echo esc_js( home_url( '/app-noleggi/' ) ); ?>';
            }).catch(function(err) {
                console.error(err);
                alert('Errore nel salvataggio dei ticket.');
            });
        });
    })();
    </script>
...
function bsn_enqueue_assets() {
    ...
    $has_tiket   = has_shortcode( $post->post_content, 'blackstar_tiket' );
    ...
    if ( $has_finalizza || $has_ispeziona || $has_rientro || $has_tiket ) {
...
function bsn_crea_ticket( $data ) {
    ...
    $result = $wpdb->insert( $table_ticket, $insert, [ '%s', '%d', '%d', '%s', '%s', '%s', '%s', '%s' ] );
