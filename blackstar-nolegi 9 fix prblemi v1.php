...
add_action('admin_menu', function() {
    ...
    add_submenu_page(
        ...
        'Finalizza noleggio',
        'Finalizza noleggio',
        'manage_options', // oppure la stessa capability che usi per i noleggi
...
function bsn_register_routes() {
    ...
    // NOLEGGI – CAMBIO STATO (SE GIÀ PRESENTE NEL FILE, LASCIA COM'È)
    register_rest_route('bsn/v1', '/noleggi/stato', [
        'methods'             => 'POST',
        'callback'            => 'bsn_cambia_stato_noleggio',
        'permission_callback' => 'bsn_check_admin',
    ]);

    // NOLEGGI – RIENTRO (attivo/ritardo -> chiuso)
    register_rest_route('bsn/v1', '/noleggi/rientro', [
        'methods'             => 'POST',
 ...
 *
 * Route: POST /bsn/v1/noleggi/rientro
 */
function bsn_api_noleggi_rientro( WP_REST_Request $request ) {
    global $wpdb;

    $table_noleggi = $wpdb->prefix . 'bs_noleggi';

    $params   = $request->get_json_params();
    $id_nol   = isset( $params['id_noleggio'] ) ? trim( (string) $params['id_noleggio'] ) : '';
    $op_rient = isset( $params['op_rientro'] ) ? trim( (string) $params['op_rientro'] ) : '';
    ...
    if ( $op_rient === '' ) {
        return new WP_REST_Response(
            array(
                'success' => false,
                'message' => 'Operatore rientro mancante.',
            ),
            400
        );
    }

    // Recupera noleggio: usa campo "id" (2026/003, ecc.)
    $noleggio = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT * FROM {$table_noleggi} WHERE id = %s LIMIT 1",
            $id_nol
    ...
    $stato_attuale = isset( $noleggio['stato'] ) ? $noleggio['stato'] : '';

    // Consenti rientro solo se attivo o ritardo
    if ( $stato_attuale !== 'attivo' && $stato_attuale !== 'ritardo' ) {
        return new WP_REST_Response(
            array(
                'success' => false,
                'message' => 'Il noleggio non è in stato attivo o ritardo.',
                'stato'   => $stato_attuale,
            ),
            400
        );
    ...
    if ( $updated === false ) {
        return new WP_REST_Response(
            array(
                'success' => false,
                'message' => 'Errore durante l\'aggiornamento del noleggio.',
            ),
            500
        );
    }
    ...
    return new WP_REST_Response(
        array(
            'success'          => true,
            'message'          => 'Noleggio chiuso correttamente.',
            'id_noleggio'      => $id_nol,
            'stato'            => 'chiuso',
            'articoli_rientro' => $articoli_rientro_json,
        ),
        200
    );
}
